...
from custom_schedulers import singular_value
...
class DenseNet3ETF(nn.Module):
    def __init__(self, depth, num_classes, growth_rate=12,
        ...
        self.features = in_planes
        if self.decl_ETF:
        #   self.W = torch.zeros((self.num_classes, self.features))
        #   self.b = torch.zeros(self.num_classes)
          self.classifier = nn.Linear(self.features, self.num_classes, bias=self.bias)
        #   self.classifier.bias = torch.zeros(self.num_classes)
          if not kwargs['args'].inference:
              self.FeaturesMovingAverage = FeaturesMovingAverageLayer(self.features, self.num_classes, device=self.device)
              self.ClosestETFGeometry = ClosestETFGeometryLayer(self.features, self.num_classes, device=self.device)
        elif self.ETF_fc:
            self.classifier = nn.Linear(self.features, self.num_classes, bias=True)
            self.classifier.bias.requires_grad_(False)
        elif self.stand_ETF:
            self.classifier = nn.Linear(self.features, self.num_classes, bias=self.bias)
        else:
            ...
            if isinstance(m, nn.Conv2d):
                ...
                if self.ETF_fc:
                    ...
                    if P is not None:
                        ...
                        m.weight = nn.Parameter(
                            torch.mm(weight, torch.eye(self.num_classes, self.features))
                        )
                    m.weight.requires_grad_(False)
        for name, module in self.named_modules():
            if isinstance(module, (nn.Linear, nn.Conv2d)) and module.weight.dim() in [2, 4]:
    ...
    def __init__(self, dataset='cifar10', depth=19, init_weights=True, cfg=None, affine=True, batchnorm=True, test_lastlayer=False, test_firstlayer=False):
        ...
        for name, module in self.named_modules():
            if isinstance(module, (nn.Linear, nn.Conv2d)) and module.weight.dim() in [2, 4]:
                ...
                module.do_prune = False
        if test_lastlayer:
            self.classifier.do_prune = False
        if test_firstlayer:
...
def vgg19(dataset='tiny_imagenet', **kwargs):
    model = VGG(dataset=dataset, depth=19, **kwargs)
    return model

def vgg16(dataset='tiny_imagenet', **kwargs):
    model = VGG(dataset=dataset, depth=16, **kwargs)
    return model

def vgg11(dataset='tiny_imagenet', **kwargs):
    model = VGG(dataset=dataset, depth=11, **kwargs)
    return model