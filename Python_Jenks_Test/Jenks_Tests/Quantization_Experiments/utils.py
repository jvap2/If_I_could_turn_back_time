...
class QuantizationObjective_v3(nn.Module):
    ...
    def forward(self):
        ...
        W = self.W*M                    # (d_out, d_in)
        Sigma = self.Sigma            # (d_in, d_in)
        Sigma_cross = self.Sigma_cross  # (d_in, d_in)
        Sigma_q = self.Sigma_q          # (d_in, d_in)
        C = self.C                    # (d_out, d_in)
        s = self.s                    # (d_out, 1)
        Q = self.Q*M                    # (d_out, d_in)
        Theta = torch.clip(torch.sigmoid(self.Theta)*1.2 -.1, 0.0, 1.0)*M  # (d_out, d_in)
        ...
        if self.second_last_layer:
            ...
            K = W @ Sigma @ W.t()
            M = W_quant @ Sigma_q @ W_quant.t()
            ## Normalize K and M by their size
            K = K / (sqrt(d_out * d_in))
            M = M / (sqrt(d_out * d_in))
            geom_loss = torch.norm(K - M, p="fro") ** 2
        if self.last_layer:
...
def run_pgd_block(model, lr=1e-2, steps=500):
    ...
    model.lamb = 1e-4  # Start with no geometric regularization, we'll increase it gradually
...
#============================================================
#         BLOCKWISE OPTIMIZATION ATTEMPT
#============================================================
...
#     def __init__(self, block_fp, block_q, cached_inputs):
#         super().__init__()
#         self.block_fp = block_fp
...
def estimate_output_curvature(model, module, dataloader, device, max_batches=2):
    ...
    for i,(x, y) in enumerate(dataloader):
        ...
        loss.backward()
        if i >= max_batches:  # limit to 10 batches for efficiency; can be increased for better estimates
            break
...
def etf_weight_loss_v2(W):
    if isinstance(W, torch.Tensor):
        ...
        raise TypeError(
            f"Unsupported type {type(W)}. "
            "Expected torch.Tensor or nn.Linear."
        )
    C = W.shape[0]
...
def count_conv_layers(module):
    return sum(1 for m in module.modules() if isinstance(m, nn.Conv2d))

def find_structural_blocks(model, min_convs=2):
    blocks = []

    for name, module in model.named_children():

        conv_count = count_conv_layers(module)

        if conv_count >= min_convs:
...
def generate_brecq_blocks(model):
    ...
    for i, (name, module) in enumerate(children):
        ...
        print(f"Analyzing module {name} for block generation...")

        # --- Stem ---
        if name == "conv1":
            blocks.append(("conv1", nn.Sequential(module)))
            print(f"--> Adding stem block: {name}")
            continue

        # --- Dense Blocks ---
        if name.startswith("block"):
            blocks.append((name, module))
            print(f"--> Adding structural block: {name}")
            continue
        ...
        # --- Final BN + ReLU (stitch into last dense block) ---
        if name in ["bn1", "relu"]:
            if len(blocks) > 0:
                last_name, last_module = blocks[-1]
                stitched = nn.Sequential(last_module, module)
                blocks[-1] = (last_name, stitched)
                print(f"--> Stitching {name} into {last_name}")
            continue

        # --- Classifier ---
        if isinstance(module, nn.Linear):
            blocks.append((name, module))
            print(f"--> Adding classifier block")
            continue
...
def generate_fine_grained_blocks(model, name):
    blocks = []
...
class BlockReconstructionObjective(nn.Module):
    ...
    def forward(self):
        ...
        for module in self.stage_q.modules():
            if isinstance(module, (QuantConv2d, QuantLinear)):
                mask = module.mask
                Theta = mask*torch.clip(torch.sigmoid(module.Theta)*1.2 -.1, 0.0, 1.0)
        ...
        if self.last_layer:
            ...
            W_quant = None
            for module, module_fp in zip(self.stage_q.modules(), self.stage_fp.modules()):
                if isinstance(module, (QuantConv2d, QuantLinear)):
            ...
            #     # geom_loss = torch.norm(K - M, p="fro") ** 2
            #     M = compute_class_means(self.cached_inputs, self.targets, num_classes=W_fp.shape[0])
            #     # print(f"self.cached_input[0]: {self.cached_inputs[0]}")
            #     # print(f"Targets: {self.targets[0]}")
            #     # print(f"Class means M computed for geometric loss.")
            #     # print(f"Class means: {M}")
            #     # geom_loss = geometry_preserve_original_weights(W_fp, W_quant)
            #     geom_loss = nc_alignment_loss(W_quant, M)
            #     print(f"Geometric loss: {geom_loss.item()}")
            #     loss += self.geo_lamb * geom_loss
        return loss
...
def geometry_aware_rounding_BRECQ(model, dataloader, device, name, bitwidth=8):
    ...
    blocks = generate_fine_grained_blocks(model, name)  # alternative fine-grained block generation
    ...
    for stage_name, block_fp in blocks:
        ...
        cached_inputs, targets = cache_block_inputs(model, block_fp, dataloader, device, num_batches=2)
